...
#### A. Annual juvenile hare trap counts 
  ...
  In order to obtain a better understanding of the inital data set, a finalized figure (Figure 1) was created to incorpate counts of juvenile hares during each year of the study. Through this figure, inital frequency assumptions about the distirbution of female and male hares though sample period could be made. 
...
# Create an initial exploratory graph of the amount of counts per year. Using ggplot() + geom_col() to create a bar graph, not a histogram. 
...
**Figure 2.** Weight (g) observations of female and male juvenile Bonanza Snowshoe hares in three sampling areas: Lowland, Mature, and Riparian. Points correspond to indviidual hare weight observations: gold (female), cyan (male), and coral (N/A, in which sex could not be determined). Overlayed box plots endpoints indicate the variation of measurents from the 1st and 3rd quartiles; the black line and point indciate the median and mean value for each sex, respectively. Data: Kielland et al. (2017)
  ...
  The figure reveals a wide distribution of observations in both the Lowland and Mature sampling sites, with the Riparian site with more frequent, centrally located data. Female and male groupings are not consistent through the sampling sites; the groups vary in their central tendency. Of the different areas, the Riparian environemnt has a relativly normal, symmetric distribution with mean and median values similar. Both the female and male hares of the the Riparian sampling site, seem to display equal normalcy, with neither group more skewed than the other.  
...
To determine the probability of the uncertantity of the means between the sexes, a two sample t-test was performed.We calculated a p-value of 0.3923. From the p-value we can determine that, there is a 39% chance that these two samples come from the same population with at least a 11.317 mean difference. This higher p-value indcates less evidence of differences between the population means. 
...
# Eexplore the strength of the correlation (degree of relationship) between two variables which, for two linearly related continuous variables, can be expressed using Pearson’s r.
...
ggplot(data = juvenile_hares, aes(x = hindft, y = weight)) +
  ...
  scale_shape_manual(values = c(16,17,18), 
                     name = "Sex") +
  geom_smooth(method = "lm", se = FALSE, color = "black") + # Use geom_smooth(method = "lm") to add a linear model to an existing scatterplot
  theme_minimal() +
